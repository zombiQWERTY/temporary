...
func NewCallManager(serviceDiscovery service_discovery.ServiceDiscovery, logger *logrus.Entry, makeServiceClient func(*grpc.ClientConn) interface{}, serviceName string) clients.CallManager {
...
func (cm *CallManagerImpl) Start() error {
	...
	services, err := cm.serviceDiscovery.GetByName(cm.serviceName) // Ex.: AUTH_SERVICE_GRPC
	if err != nil {
		return err
	}
...
func (cm *CallManagerImpl) registerConnection(v *connections_pool.ServiceConnection) {
	...
	host := strings.Replace(v.Host, "http://", "", 1)
	client, err := NewCallConnection(v.Id, fmt.Sprintf("%s:%d", host, v.Port), cm.makeServiceClient)
	if err != nil {
