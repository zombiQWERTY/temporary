...
func NewDirectoriesService(serviceDiscovery service_discovery.ServiceDiscovery) *DirectoriesService {
	return &DirectoriesService{
		serviceDiscovery: serviceDiscovery,
	}
}

func (u *DirectoriesService) StartCM(logger *logrus.Entry) (postgres_client.CallManager, error) {
	cm := postgres_client.NewCallManager(u.serviceDiscovery, logger, model.DIRECTORIES_POSTGRES_SERVICE_NAME, model.DIRECTORIES_POSTGRES_USER_KEY, model.DIRECTORIES_POSTGRES_PASSWORD_KEY, model.DIRECTORIES_POSTGRES_DATABASE_NAME_KEY)
	err := cm.Start()
	if err != nil {
		return nil, err
	}
...
func (u *DirectoriesService) GetDB(dbName string) (*gorm.DB, error) {
	if u.cm == nil {
...
func (u *DirectoriesService) CreateDatabase(dbName string) (*gorm.DB, error) {
