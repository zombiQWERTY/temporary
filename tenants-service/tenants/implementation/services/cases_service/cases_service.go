...
func NewCasesService(serviceDiscovery service_discovery.ServiceDiscovery) *CasesService {
	return &CasesService{
		serviceDiscovery: serviceDiscovery,
	}
}

func (u *CasesService) StartCM(logger *logrus.Entry) (postgres_client.CallManager, error) {
	cm := postgres_client.NewCallManager(u.serviceDiscovery, logger, model.CASES_POSTGRES_SERVICE_NAME, model.CASES_POSTGRES_USER_KEY, model.CASES_POSTGRES_PASSWORD_KEY, model.CASES_POSTGRES_DATABASE_NAME_KEY)
	err := cm.Start()
	if err != nil {
		return nil, err
	}
...
func (u *CasesService) GetDB(dbName string) (*gorm.DB, error) {
	if u.cm == nil {
...
func (u *CasesService) CreateDatabase(dbName string) (*gorm.DB, error) {
