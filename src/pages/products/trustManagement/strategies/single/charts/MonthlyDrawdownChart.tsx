...
const getFilteredDataFromPeak = (monthlyData: Record<string, any>[]) => {
  const maxMonthlyData = Math.max(...monthlyData.map((d) => d.value));
  const maxDate = monthlyData.find((d) => d.value === maxMonthlyData)?.month;

  let previousValue: number | null = null;

  return monthlyData.map((d) => {
    const currentDate = parseISO(`${d.month}-01`);
    const maxDateParsed = parseISO(`${maxDate}-01`);

    if (isAfter(currentDate, maxDateParsed)) {
      if (previousValue !== null) {
        const drawdown =
          previousValue > d.value
            ? (previousValue - d.value) / previousValue
            : 0;
        previousValue = d.value;
        return { ...d, value: drawdown };
      } else {
        previousValue = d.value;
        return d;
      }
...
interface MonthlyDrawdownChartProps {
  strategy: GetStrategyByIdApi.StrategySchema;
}

// @FIXME: Тестовый график. Вообще хз, что тут происходит
export const MonthlyDrawdownChart = ({
  strategy,
}: MonthlyDrawdownChartProps) => {
  const data = useMemo(
    () =>
      getFilteredDataFromPeak(
  ...
  const strategyData = useMemo(() => {
    const d = getFilteredDataFromPeak(
      strategy.monthlyReturn.map((r) => ({
        month: `${r.year}-${String(r.month).length === 1 ? `0${r.month}` : r.month}`,
        value: r.return,
      })),
    );
    ...
    if (strategyDataLength < dataLength) {
      ...
      return [...zerosArray, ...d.map((d) => d.value)];
    }
